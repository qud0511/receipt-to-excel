...
## Phase 4.5 — Smoke Gate 보강 (2026-05-12, 추가 세션)

직전 1차 smoke 26% 결과 분석 → 4 결함 식별 → 본 세션에서 결함 1, 2 해소.

### 산출

| 영역 | 결정 | ADR / Commit |
| --- | --- | --- |
| 결함 1: PDF 한글 byte 매칭 불가 | text-aware provider 감지 — 추출 텍스트 시그니처 매칭 + 우리 N-up dual gate | ADR-007 (51cf8a6) |
| 결함 1b: 압축 stream PDF heuristic | `is_text_embedded()` `BT` 단일 토큰 → `BT OR /Font OR /ToUnicode` OR 결합 — woori_nup_02 활성화 | (4788778) |
| 결함 2: rule_based 정규식 layout 불일치 | shinhan: 라벨 강제 + dot/dash + \x01 + 초 옵셔널 + bare line 금액 fallback + 가맹점 정보 블록 fallback. kbank: `\s+` → `\s*` (일/시각, 숫자/원). | ADR-008 (236318c, 9807d05) |
| 결함 3: OCR LLM 가맹점명 empty | 한계 격리 + 별도 트랙 명시 (Phase 4.6 또는 Phase 5 후반 ADR-009) | docs/limitations/ocr_qwen_vendor_name.md |

### Smoke Gate 최종 결과 (3차 실행)

`tests/smoke/results/20260512.md` (PII 마스킹, gitignore).

| 회차 | 적용 | PASSED | Δ |
| --- | --- | --- | --- |
| 1차 | (baseline) | 11 / 42 = 26 % | — |
| 2차 | ADR-007 + is_text_embedded 보강 | 17 / 42 = 40.5 % | +14.5 pp |
| **3차 (최종)** | + ADR-008 정규식 보강 | **37 / 42 = 88.1 %** | **+47.6 pp** |

### 카드사별 분포 (3차)

| Provider | Pass | Method | latency |
| --- | --- | --- | --- |
| shinhan (일반 12 + 택시 6) | 18 / 18 | rule_based | 0.2-0.5s |
| samsung | 8 / 8 | rule_based | 0.1-0.3s |
| kbank | 5 / 5 | rule_based | 0.1s |
| woori (N-up 9 거래) | 2 / 2 | rule_based | 0.1-0.2s |
| hyundai | 1 / 1 | ocr_hybrid | 44s |
| lotte | 1 / 1 | ocr_hybrid | 47s |
| woori_legacy | 2 / 2 | ocr_hybrid | 37-39s |
| hana (JPG) | 0 / 2 | OCR fail | (결함 3) |
| kakaobank (JPG) | 0 / 3 | OCR fail | (결함 3) |
| **합계** | **37 / 42 = 88.1 %** | rule_based 33 / ocr 4 | |

### 테스트 카운트 (Phase 4 + 4.5 누적)

| 영역 | 카운트 | Δ Phase 4.5 |
| --- | --- | --- |
| 단위 | **161** | +15 (pdf_text_probe 7 + shinhan 3 + kbank 2 + 기타 회귀) |
| 통합 | 3 (skip 1) | 0 |
| smoke (real_pdf) | 42 | 0 |
| **합계** | **206 (skip 1)** | +15 |

mypy --strict + ruff + pip-audit 통과. 회귀 0 건.

### Phase 5 진입 게이트 — **70-90% 구간 → 사용자 결정 대기**

사용자 정의 임계 (CLAUDE.md §"카드사 양식 변동성"):

- 90 %+ → Phase 5 진입 승인
- **70-90 % → docs/limitations + Phase 5 진입 검토 (사용자 결정 요청)** ← 현재
- < 70 % → 멈춤

현재 88.1 % → docs/limitations/ocr_qwen_vendor_name.md 작성 완료. Phase 5 진입은
**사용자 판단**:

- **(권장) Phase 5 진입 + ADR-009 별도 트랙**: 잔여 5건 (hana 2 + kakaobank 3) 은
  모두 JPG 이미지 OCR LLM 한계. Phase 5 (Templates + XLSX) 와 직교. 운영 시 수동
  가맹점명 입력 + vendor_matcher 자동완성으로 대응 가능.
- **(대안) ADR-009 즉시 처리 후 Phase 5**: qwen2.5vl 빈 응답 retry 또는 OCR hint
  fallback (~1-2 시간) → 90%+ 도달 후 Phase 5 진입.

---

> Phase 4 + 4.5 최종 상태: **단위 206 (skip 1) 통과 + Smoke 88 % + ADR 8건 (003~008) 완료**. Phase 5 진입 검토 단계.
